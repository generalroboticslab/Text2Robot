...
def write_link_urdf(joints_dict, repo, links_xyz_dict, file_name, inertial_dict, ui):
    """
    ...
    with open(file_name, mode='a') as f:
        ...
        link.make_link_xml(isInvisible=0)
        ...
        for joint in joints_dict:
            ...
            isInvisible = 0
            if 'invis' in name:
                ui.messageBox("Found invisible component")
                isInvisible = 1
            ...
            links_xyz_dict[link.name] = link.xyz            
            link.make_link_xml(isInvisible)
            f.write(link.link_xml)
            f.write('\n')
...
def write_urdf(joints_dict, links_xyz_dict, inertial_dict, package_name, save_dir, robot_name, ui):
    file_name = save_dir + '/' + robot_name + '.urdf'  # the name of urdf file
    # change to "../../URDF_Bank/name(RobotDog_top-x-6_bottom-y-5)/simple_meshes"
    repo = 'simple_meshes/'  # Pybullet only need relative paths
    with open(file_name, mode='w') as f:
        f.write('<?xml version="1.0" ?>\n')
        f.write('<robot name="{}">\n'.format(robot_name))
        f.write('\n')
        f.write('<material name="pla_5_percent_infill">\n')
        f.write('  <color rgba="0.700 0.700 0.700 1.000"/>\n')
        f.write('</material>\n')
        f.write('\n')

    write_link_urdf(joints_dict, repo, links_xyz_dict, file_name, inertial_dict, ui)
