...
class Joint:
    def __init__(self, name, xyz, axis, parent, child, joint_type, upper_limit, lower_limit):
        ...
        """
        self.name = name[0:len(name)-2]+"_"+name[-1]
        ...
        self.upper_limit = 1.2 #upper_limit  # for 'revolute' and 'prismatic'
        self.lower_limit = -1.2  # for 'revolute' and 'prismatic'
    ...
    def make_joint_xml(self):
        ...
        # ui.messageBox("Joint type = "+self.type)
        self.type = 'revolute'

        joint = Element('joint')
        joint.attrib = {'name':self.name, 'type':self.type}

        # if self.parent == "base_link":
        #     parentDisplayName = self.parent
        # else:
        #     parentDisplayName = self.parent[:-2]

        # if self.child == "base_link":
        #     childDisplayName = self.child
        # else:
        #     childDisplayName = self.child[:-2]
